...
from metrics.JS_eval import JS_eval
        ...
        while not self.exit_flag.is_set():
            ...
            for sock_idx, sock in enumerate(socks):
                if sock in events:
                    ...
                    #msg = [['generated summary 1'], [['ref summary 1'], ['ref summary 2']], 'rouge_n']]
                    ...
                    for index,_ in enumerate(msg):
                        summary = msg[index][0]
                        references_text = msg[index][1]
                        eval_metric = msg[index][2]
                        if eval_metric[:-1] == 'rouge_':
                            if eval_metric[-1] == '1' or eval_metric[-1] == 'n':
                                score = rouge_n(summary, references_text, 1, alpha=0)
                            if eval_metric[-1] == '2':
                                score = rouge_n(summary, references_text, 2, alpha=0)
                        if eval_metric == 'JS_eval':
                            score = JS_eval(summary, references_text, 1)

                        #sink.send_multipart([client_id, jsonapi.dumps(msg), b'', ServerCmd.data_pair])
